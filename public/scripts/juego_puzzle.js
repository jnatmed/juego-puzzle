...
class Juego {
    ...
    constructor(opcionesJuego){
      this.piezas = opcionesJuego.piezas;
      this.puzzle = opcionesJuego.puzzle;
      this.terminado = opcionesJuego.terminado;
      this.currentCanvas = null;
      this.estadoPiezas = [];
      this.estadoPiezas.fill(-1);
      this.estadoPuzzle = [];
      this.estadoPuzzle.fill(-1);
      this.url_data = opcionesJuego.url_data;
      this.aciertos = 0;
      this.errores = 0;
      this.id_partida = opcionesJuego.id_partida;
      this.id_usuario = opcionesJuego.id_usuario;
  
      this.minutos = 0;
      this.segundos = 0;
      this.intervalId = opcionesJuego.intervalId;
    ...
    getIdPartida(){ return this.idPartida; }
    setIdPartida(idPartida) { this.idPartida = idPartida; }
    ...
    isValidDataURI(dataURI) {
      // Un Data URI válido debe tener el formato "data:[<mediatype>][;base64],<data>"
      // Verificamos si la cadena comienza con "data:" y contiene al menos una coma ","
      return typeof dataURI === 'string' && dataURI.startsWith('data:') && dataURI.includes(',');
    }
  
    async enviarDataURIAlBackend(dataURIRecibida, datos) {
  
      if (this.isValidDataURI(dataURIRecibida)) {
        try {
          ...
          const response = await fetch('/guardar_imagen', {
            ...
            body: JSON.stringify({ 
              dataURI : dataURIRecibida,
              id_partida : datos.id_partida,
              id_usuario : datos.id_usuario
            }), // Convierte el dataURI en una cadena JSON
          });
  
          if (response.ok) {
            const resultado = await response.text(); // Si el servidor responde con JSON
        ...
        console.error('El Data URI no es válido.');
      }
    }  
...
$('btn_cargar').addEventListener('click', async (e) => {
  ...
  try {
    ...
    const opcionesJuego = {
      ...
      url_data : url_data,
      intervalo: intervalo,
      id_partida: $('id_partida').getAttribute('data-value'),
      id_usuario: $('id_usuario').getAttribute('data-value')
    };

    const juego = new Juego(opcionesJuego);
    ...
    await juego.enviarDataURIAlBackend(dataURI, opcionesJuego);
